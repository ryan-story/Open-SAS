"""
Command Line Interface for Open-SAS

This module provides a CLI for running .osas files from the command line.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Open-SAS: Python-based SAS interpreter",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  open-sas program.osas          # Run a .osas file
  open-sas -i                    # Interactive mode
  open-sas --version             # Show version
        """
    )
...
def run_file(interpreter: SASInterpreter, file_path: str, verbose: bool = False):
    ...
    if not file_path.endswith('.osas'):
        print(f"Warning: File '{file_path}' does not have .osas extension.", file=sys.stderr)
...
def run_interactive(interpreter: SASInterpreter):
    ...
    print("Open-SAS Interactive Mode")
