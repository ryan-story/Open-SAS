...
export class OSASNotebookProvider implements vscode.NotebookProvider {
    ...
    async executeCell(document: vscode.NotebookDocument, cell: vscode.NotebookCell): Promise<void> {
        ...
        try {
            ...
            if (result.output) {
                outputItems.push(
                    vscode.NotebookCellOutputItem.text(result.output, 'text/x-sas-output')
                );
            }
            ...
            if (result.errors) {
                outputItems.push(
                    vscode.NotebookCellOutputItem.text(result.errors, 'text/x-sas-error')
                );
            }
            ...
            const errorOutput = new vscode.NotebookCellOutput([
                vscode.NotebookCellOutputItem.text(
                    `Error executing SAS code: ${error}`,
                    'text/x-sas-error'
                )
            ]);
    ...
    private async executeSASCode(code: string, notebookPath: string): Promise<any> {
        ...
        if (head && head.length > 0) {
            html += `
                <table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
                    ...
                    <tbody>
                        ${head.map((row: any) => 
                            `<tr>${columns.map((col: string) => `<td style="border: 1px solid #ddd; padding: 8px;">${row[col] || ''}</td>`).join('')}</tr>`
                        ).join('')}
                    </tbody>
                </table>
            `;
    ...
    private createPROCResultHTML(procResult: any): string {
        ...
        if (procResult.data) {
            html += `
                <table style="border-collapse: collapse; width: 100%; font-size: 0.9em;">
                    <tbody>
                        ${procResult.data.map((row: any) => 
                            `<tr>${Object.values(row).map((val: any) => `<td style="border: 1px solid #ddd; padding: 8px;">${val}</td>`).join('')}</tr>`
                        ).join('')}
                    </tbody>
                </table>
            `;
